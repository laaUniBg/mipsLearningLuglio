...
main:
	...
	la $t1, second 	#leggo indirizzo double nella ram
	
	ldc1 $f4, 0($t0) # tutti gli $f tranne dispari e 0, 2,12,14... 0 e 2 sono come return $v e 12,14 come arguments $a
	ldc1 $f6, 0($t1) # tutti gli $f tranne dispari e 0, 2,12,14... 0 e 2 sono come return $v e 12,14 come arguments $a
	...
	la $t3, risultato # indirizzo ram risultato
	sdc1 $f8, 0($t3) # scrivo il risultato nella ram
	
	li $v0, 3 # write double code
	ldc1 $f12, 0($t3) # uso indirizzo di risultato che ho già riempito con store
	syscall
